...
def _modinv(a: int, pk: int) -> Optional[int]:
...
def lift_root(a: int, p: int, k: int) -> Optional[int]:
    """
...
def compare_methods(p: int, k: int, n_trials: int = 20) -> Dict[str, float]:
    ...
    for _ in range(n_trials):
        ...
        for name, step_fn in methods.items():
            ...
            if prof:
                results[name].append(float(prof[-1]) if prof[-1] != float("inf") else float(k))
...
def verify_order(
    primes: List[int] = None, k: int = 8, n_trials: int = 10
...
def newton_correction_uniformity(
    ...
    δ = (x^3 - a) / (3 x^2) mod p, and the claim is this is
