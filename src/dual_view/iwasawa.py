...
def ldu_decompose(M: List[List[int]], k: int) -> Optional[Dict[str, List[List[int]]]]:
    """
...
class MatrixCoordinates:
    ...
    trace_depth: int
    det_dual: DualNumber
    ldu: Optional[Dict[str, List[List[int]]]]
...
def matrix_coordinates(M: List[List[int]], k: int) -> MatrixCoordinates:
    ...
    trace_val = (M[0][0] + M[1][1]) & _mask(k)
    trace_depth = _valuation(trace_val) if trace_val != 0 else k
    det_val = _mat_det(M, 1 << k)
...
def matrix_commutator(
    ...
    MN = _mat_mul(M, N, mod)
    MN_inv = _mat_mul(inv_M, inv_N, mod)
    return _mat_mul(MN, MN_inv, mod)
