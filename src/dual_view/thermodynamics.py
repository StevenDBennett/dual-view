...
stability measure (the ghost ratio encodes only the α-bit).
Higher v₂(e_true) means the weight lies deeper in the 2-adic
congruence filtration, hence more stable under quantization.
...
class SeedThermodynamics:
    ...
    1. **Coordinate analysis** (from v1): given a weight matrix,
       compute comprehensive 2-adic statistics via `analyse(W)`.

    2. **Precision-sweep analysis** (from v2): given weights + k_range,
       find ghost cliffs per weight via `compute()`.

    Parameters
    ----------
    k : int
        Bit precision for coordinate analysis.
    g : int
    ...
    def weight_coordinates(self, w: int) -> Tuple[float, int, int]:
        """
        2-adic dual coordinates (v, α, e) for a single weight.

        Returns (v, α, e) where v can be float('inf') for zero.
        """
        w_masked = w & self.mask
        if w_masked == 0:
            return (float("inf"), 0, 0)
        v = _valuation(w_masked)
        if v >= self.k:
            return (float("inf"), 0, 0)
        odd = (w_masked >> v) & self.mask
    ...
    def analyse(self, W: np.ndarray) -> Dict[str, float]:
        ...
        for w in W.flat:
            ...
            if w_int & 1 == 0:
                continue
            n_odd += 1
            v, alpha, e = self.weight_coordinates(w_int)
            alpha_frac += alpha
            if isinstance(v, float) and v == float("inf"):
                continue
            # Compute v2(e_true) via dlog
            odd = abs(w_int) & self.mask
            result = two_adic_dlog(odd, self.k)
            if result is not None:
                _, e_true = result
                v2_e = _valuation(e_true) if e_true != 0 else self.k
        ...
        n_mersenne_zone = sum(1 for v in v2_e_vals if v >= self.k // 2)
    ...
    def mersenne_cliff_score(self, w: int) -> Optional[int]:
        """
        Minimum precision k* needed for stable quantization of weight w.

        Returns k* = v₂(e_true) + 2, or None if w is even.
        """
        if w & 1 == 0:
            return None
        odd = abs(w) & self.mask
    ...
    def __call__(self, weights: np.ndarray, k_range: range) -> "SeedThermodynamics":
        """Convenience: create a new instance configured for precision sweep."""
        self._weights = weights
        self._k_range = k_range
        self._profiles: Optional[Dict] = None
        self._cliffs: Optional[Dict] = None
        return self
    ...
    def stable_weights(self, max_k: Optional[int] = None) -> List[int]:
        """
        Indices of weights with no ghost cliff (or cliff ≥ max_k).

        Parameters
        ----------
        max_k : int, optional — if given, returns weights whose cliff
                precision is >= max_k (i.e. stable at max_k bits).
        """
    ...
    def ghost_weights(self, max_k: Optional[int] = None) -> List[int]:
        """
        ...
        ----------
        max_k : int, optional — if given, returns weights whose cliff
                precision is < max_k (i.e. ghostly below max_k bits).
        """
        if max_k is not None:
            return [idx for idx, c in self.cliffs.items()
