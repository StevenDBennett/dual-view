...
from .nonabelian import NonAbelianCRTDual, _mat_mul, _mat_det
...
def verify_isometry(k: int, n_trials: int = 200) -> Dict[str, float]:
    ...
    for _ in range(n_trials):
        ...
        if diff == 0:
            ...
            v2_diff = _valuation(diff)
        v2_e = _valuation(e) if e != 0 else k
        expected = v2_e + 2
...
def isometry_pair_test(k: int, n_trials: int = 200) -> Dict[str, float]:
    ...
    for _ in range(n_trials):
        ...
        v2_e = _valuation(e_diff) if e_diff != 0 else k
        expected = v2_e + 2
        if v2_diff == expected or v2_diff >= k:
            passed += 1
    return {"pass_rate": passed / n_trials, "n_trials": n_trials}
...
def verify_operator_algebra(k_values: List[int] = None, n_random_fns: int = 5) -> Dict[str, bool]:
    """
    Verify average and difference operator identities.

    Checks: avg^2 = N·avg, D·avg = 0, avg·D = 0.
    """
    if k_values is None:
        k_values = [5, 6, 7, 8]

    results: Dict[str, bool] = {
        "avg_sq_eq_N_avg": True,
        "D_avg_eq_0": True,
        "avg_D_eq_0": True,
    }

    for k in k_values:
        ctx = OperatorContext(k, 5)
        N = ctx.N

        # Use a random test function
        for _ in range(n_random_fns):
            test_fn = lambda e, r=random: r.randint(0, (1 << k) - 1)

            lhs1 = (ctx.avg * ctx.avg)(test_fn, 0)
            rhs1 = N * ctx.avg(test_fn, 0)
            if (lhs1 & ctx.mask) != (rhs1 & ctx.mask):
                results["avg_sq_eq_N_avg"] = False

            lhs2 = (ctx.diff * ctx.avg)(test_fn, 0)
            if lhs2 != 0:
                results["D_avg_eq_0"] = False

            lhs3 = (ctx.avg * ctx.diff)(test_fn, 0)
            if lhs3 != 0:
                results["avg_D_eq_0"] = False
...
def exponent_valuation_profile(k: int, n_samples: int = 500) -> Dict[int, float]:
    """
    ...
    for _ in range(n_samples):
        ...
        if w & 1 == 0:
            continue
        w_adj = w if (w & 3) == 1 else (-w) & _mask(k)
        result = two_adic_dlog(w_adj, k)
        if result is not None:
            _, e_true = result
            v2 = _valuation(e_true) if e_true != 0 else k - 2
