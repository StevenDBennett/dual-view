...
from dual_view.core import (
    padic_exp, padic_log, g0, _valuation, _mask,
)
from dual_view.mahler import MahlerCalculus


# ── padic_exp / padic_log ─────────────────────────────────────────────────────

class TestPadicExp(unittest.TestCase):
    """Tests for the general 2-adic exponential."""

    def test_exp_zero(self):
        self.assertEqual(padic_exp(0, 8), 1)

    def test_exp_minus4(self):
        """exp(−4) mod 2^k is the cliff centre g₀."""
        for k in [8, 16, 32]:
            result = padic_exp(-4, k)
            self.assertEqual(result % 4, 1, f"exp(−4) should be ≡ 1 mod 4 at k={k}")

    def test_exp_log_roundtrip(self):
        """exp(log(g)) ≡ g mod 2^k for g ≡ 1 mod 4."""
        for k in [8, 16]:
            mod = 1 << k
    ...
    def test_exp_requires_v2_ge_2(self):
        """exp should reject x with v₂(x) < 2."""
    ...
    def test_log_requires_v2_ge_2(self):
        """log should reject g with v₂(g−1) < 2."""
        with self.assertRaises(ValueError):
            padic_log(3, 8)   # v₂(2) = 1
        with self.assertRaises(ValueError):
            padic_log(2, 8)   # v₂(1) = 0
    ...
    def test_exp_additive_property(self):
        """exp(x+y) ≡ exp(x)·exp(y) mod 2^k (when both converge)."""
        k = 16
        mod = 1 << k
        x, y = 4, 8
        exp_x = padic_exp(x, k)
    ...
    def test_exp_at_k32(self):
        """exp(−4) at k=32 should be consistent with lower k."""
        g0_16 = padic_exp(-4, 16)
        g0_32 = padic_exp(-4, 32)
        self.assertEqual(g0_32 & _mask(16), g0_16)


# ── g0 (cliff centre) ────────────────────────────────────────────────────────

class TestG0(unittest.TestCase):
    """Tests for the cliff centre g₀ = exp₂(−4)."""

    def test_g0_mod_4_is_1(self):
        """g₀ ≡ 1 mod 4 (it is a 2-adic unit in the log domain)."""
        for k in [8, 16, 32]:
    ...
    def test_g0_agrees_with_neg123_to_13_bits(self):
        ...
        self.assertEqual(_valuation(diff), 13)


# ── MahlerCalculus ────────────────────────────────────────────────────────────

class TestMahlerPolynomial(unittest.TestCase):
    """Tests for Mahler polynomial computation."""
...
class TestMahlerTransform(unittest.TestCase):
    """Tests for to_mahler / from_mahler roundtrip."""
...
class TestDiracOperator(unittest.TestCase):
    """Tests for the Dirac (forward-difference) operator."""
...
class TestVolterraOperator(unittest.TestCase):
    """Tests for the Volterra (right-inverse) operator."""
...
class TestDT_TD_Asymmetry(unittest.TestCase):
    """
    Tests for the precise boundary behaviour of D and T.
...
class TestTruncate(unittest.TestCase):
    """Tests for coefficient truncation."""
...
class TestCliffDensityTheory(unittest.TestCase):
    """
    THEOREM (Cliff Density):
    For a uniformly random log-target L mod 2^k:

        Pr[c = 0] = 7/8          (no cliff, 87.5% of targets)
        Pr[c = j] = 2^{-(j+3)}   for j ≥ 1

        E[c] = 1/4  (expected cliff cost: only 0.25 extra bits on average)
    """

    def test_cliff_distribution_exact(self):
        """Pr[c=j] = 2^{-(j+3)} for j≥1; Pr[c=0] = 7/8."""
        k = 14
        total = 1 << (k - 2)
        counts = {}
        for L_val in range(total):
            ...
            lp4 = L_signed + 4
            c = max(0, _valuation(abs(lp4)) - 2) if lp4 != 0 else k
    ...
    def test_expected_cliff_cost_is_quarter(self):
        ...
        total = 1 << (k - 2)
        total_c = sum(
            max(0, _valuation(abs((L - total if L > total // 2 else L) + 4)) - 2)
...
class TestDivisorOptimality(unittest.TestCase):
    ...
    def test_all_divisors_all_j(self):
        ...
        for d in range(6):
            for j in range(2, 7):
                ...
                v_new = _valuation(diff) if diff else k
                if d < 2:
                    pred = j
                elif d == 2:
                    pred = min(2 * j + 1, k - 1)
    ...
    def test_d2_is_strictly_best(self):
        """d=2 strictly outperforms all other d for j ≥ 2."""
        k = 20
        mod_e = 1 << (k - 2)
        target_e = 333 % mod_e
        target = pow(5, target_e, 1 << k)
        for j in range(2, 7):
            e = (target_e + (1 << j)) % mod_e
            v_d2 = _valuation((self._step(e, target, k, 2) - target_e) % mod_e)
            for d in [0, 1, 3, 4]:
                e_new = self._step(e, target, k, d)
                if e_new is None:
                    continue
                diff = (e_new - target_e) % mod_e
                v_d = _valuation(diff) if diff else k
